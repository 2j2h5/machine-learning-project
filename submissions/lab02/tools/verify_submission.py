...
def main():
    ...
    archive = ROOT / f"{lab02.STUDENT_ID}_{lab02.STUDENT_NAME}_lab02.zip"
    with ZipFile(archive) as zipped:
        expected_names = {"src/lab02.py", "report.pdf", "results.json", "README.md"}
        ...
        assert expected["student_id"] == lab02.STUDENT_ID and expected["name"] == lab02.STUDENT_NAME
        assert expected["seed"] == 42 and expected["lab"] == "lab02"
        ...
        assert lab02.STUDENT_ID in text and lab02.STUDENT_NAME in text
        assert "PREVIEW" not in text and "pending" not in text and "{" not in text
        for required in ("Figure 1", "Figure 2", "Figure 3", "Table 1", "Table 2",
                         "Interpretation", "Limitations", "AI & external-code usage"):
            assert required in text, required
        with tempfile.TemporaryDirectory(prefix="lab02-zip-check-") as temp:
            work = Path(temp)
            ...
            subprocess.run([args.python, "-I", "-B", "-X", "utf8", "src/lab02.py"],
