...
def build(preview=False):
    ...
    while i < len(lines):
        ...
        if line == "<!-- pagebreak -->":
            ...
            while i < len(lines) and lines[i].strip().startswith("|"):
                ...
                i += 1
            fractions = [0.65, 0.35] if len(rows[0]) == 2 else [0.21, 0.37, 0.42]
    ...
    preview_dir = Path(tempfile.gettempdir()) / "machine-learning-project" / "lab01-pdf-preview"
    ...
    def footer(canvas, doc):
        ...
        canvas.drawString(44, 23, "Lab 1 | Cafe sales EDA | Seed 42")
    ...
    doc = SimpleDocTemplate(str(output), pagesize=A4, rightMargin=44, leftMargin=44,
                            ...
                            title="Lab 1 - Cafe sales EDA", author=results["name"])
    ...
    print(f"Report: {output} ({len(reader.pages)} text-based pages)")
