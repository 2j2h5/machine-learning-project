...
STUDENT_ID = "20201068"   # TODO: your student id, e.g. "20261234"
STUDENT_NAME = "이지호"     # TODO: your name in Korean or roman letters — "홍길동" / "HongGildong"
...
def summarize_missing(df: pd.DataFrame) -> pd.Series:
    ...
    """
    counts = df.isna().sum()
    return counts[counts > 0].sort_values(ascending=False, kind="stable")
# ============================ END TODO (Task 1) ==============================
...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    cleaned = df.drop_duplicates(keep="first").reset_index(drop=True).copy()
    cleaned["unit_price"] = (
        cleaned["unit_price"].astype(str).str.replace(",", "", regex=False).astype(float)
    )

    median_quantity = cleaned["quantity"].median()
    cleaned["quantity"] = cleaned["quantity"].fillna(median_quantity).astype(int)

    missing_total = cleaned["total_price"].isna()
    cleaned.loc[missing_total, "total_price"] = (
        cleaned.loc[missing_total, "unit_price"] * cleaned.loc[missing_total, "quantity"]
    )

    mean_rating = round(cleaned["customer_rating"].mean(), 2)
    cleaned["customer_rating"] = cleaned["customer_rating"].fillna(mean_rating)
    return cleaned
# ============================ END TODO (Task 2) ==============================
...
def detect_outliers_iqr(df: pd.DataFrame, column: str, k: float = 1.5) -> list:
    ...
    iqr = q3 - q1
    outside = (values < q1 - k * iqr) | (values > q3 + k * iqr)
    return sorted(int(label) for label in df.index[outside])
# ============================ END TODO (Task 3) ==============================
