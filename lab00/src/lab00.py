...
def sigmoid(z: np.ndarray) -> np.ndarray:
    ...
    # Each branch keeps the exponent non-positive to avoid overflow.
...
def softmax_loop(z: list) -> list:
    ...
    exp_values = []
    for value in z:
        exp_values.append(math.exp(value - largest))
...
def cross_entropy(p: np.ndarray, q: np.ndarray) -> float:
    ...
    """
    nonzero = p > 0
    safe_q = np.maximum(q[nonzero], 1e-12)
    return float(-np.sum(p[nonzero] * np.log(safe_q)))
# ============================ END TODO (Task 5) ==============================
...
def kl_divergence(p: np.ndarray, q: np.ndarray) -> float:
    ...
    """
    nonzero = p > 0
    positive_p = p[nonzero]
    safe_q = np.maximum(q[nonzero], 1e-12)
    divergence = np.sum(positive_p * (np.log(positive_p) - np.log(safe_q)))

    # Rounding and the probability floor can produce a tiny negative value.
...
def focal_loss(p: np.ndarray, q: np.ndarray, gamma: float = 2.0,
    ...
    nonzero = p > 0
    safe_q = np.maximum(q[nonzero], 1e-12)
    focusing_weight = (1.0 - q[nonzero]) ** gamma
    losses = -p[nonzero] * focusing_weight * np.log(safe_q)
    if alpha is not None:
        losses = losses * alpha[nonzero]
